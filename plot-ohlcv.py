...
def main():
    ...
    df["date"] = df["open_timestamp_utc"].map(lambda x: datetime.fromtimestamp(x))
    
    w = (df["open_timestamp_utc"][1] - df["open_timestamp_utc"][0]) * 1000 * 0.5
    ...
    p = figure(x_axis_type="datetime", tools=TOOLS, plot_width=1000, title = "OHLCV")
